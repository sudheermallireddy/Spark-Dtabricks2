...
class SiteMoglakwenaFms(RawTransformationFactory):
  
    def read_data(self, zone, store_type, file_format, input_path):      
      logger.info(TAG_MOG_FMS,"Reading data for site Moglakwena with file format : {} at {}".format(file_format, input_path))
    ...
    def write_data(self, store_type, file_format, output_path, transformed_df):
      logger.info(TAG_MOG_FMS,"Writing data for site Moglakwena in file format :{} at {} with store type : {}".format(file_format,output_path, store_type))
