...
TAG_MR = "MinasRioTransformationsFms" 
...
def mr_transform_tbl_Equipment(base_input_path, site_executor, input_config):
  ...
  logger.info(TAG_MR,"reference_input_path : "+reference_input_path)
  
  try:
    aux_path = get_final_input_path(base_input_path, MR_STD_SHIFT_AUX)
    ...
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {} : {}".format(aux_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format(aux_path))

  try:
    eqmt_path = get_final_input_path(base_input_path, MR_STD_SHIFT_EQMT)
    std_shift_eqmt_df = site_executor.read_data(CURATED, ADLS2, PARQUET, eqmt_path)\
                                     .select("FieldId", "UnitId", "Unit", "UnitType", "EqmttypeId", "Eqmttype","Site_Name","Src_Application")
  except Exception as e:
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {} : {}".format(eqmt_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format(eqmt_path))
  ...
  try:
    ...
    logger.error(TAG_MR,"TransformationException","Failed to perform union operation for table Equipment : {}".format(str(e)))
    raise DataTransformationException(TAG_MR,"TransformationException",\
  ...
  try:
    ...
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {} : {}".format(catalog_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format(catalog_path))
    

  #performing join based on logical model
  try:
    site_equipment_df = std_eqmt_catalog_df.filter((upper(std_eqmt_catalog_df.SRC_System) == lit(site_name.upper().replace("_", " "))) & \
    ...
    tbl_Equipment_df =  site_equipment_df.join(std_all_eqmt_df, (std_all_eqmt_df.FieldId == trim(site_equipment_df.Plant_No)), "fullouter") \
                   ...
                   .withColumn("Site_Name",lit(site_name.upper().replace("_", " "))) \
    ...
    logger.error(TAG_MR,"TransformationException","Failed to perform join operation for table Equipment : {}".format(str(e)))
    raise DataTransformationException(TAG_MR,"TransformationException",\
...
def mr_transform_tbl_equipment_status(base_input_path, site_executor, input_config):
  site_name=input_config.site
  # Getting Equipment Status tables
  eqmt_status_path = get_final_input_path(base_input_path, MR_STD_SHIFT_STATE)
  eqmt_status_df = site_executor.read_data(CURATED, ADLS2, PARQUET, eqmt_status_path)
  eqmt_status_df_filter = eqmt_status_df.select("FieldId","ShiftId","Timestamp","Duration","Eqmt",\
                        "AuxEqmt","FieldTime","FieldReason","TimecatId","FlagCodes",\
                        "StatusIdx","Status","FieldComment","OperId","IsAuxil","Site_Name","Src_Application").filter(col("ShiftId") > 171231003)
  shift_info_df = mr_transform_tbl_shift_info(base_input_path, site_executor, input_config).where(col("ShiftId").isNotNull()) .select('ShiftId','Crew').withColumnRenamed('ShiftId', 'ShiftId_df')
  
  try:
    joined_equip_status_df = eqmt_status_df_filter.join(shift_info_df, eqmt_status_df_filter.ShiftId == shift_info_df.ShiftId_df ,'left') \
                                           .withColumn('Status_Start_Time',from_unixtime(unix_timestamp('FieldId','HH:mm:ss'),'HH:mm:ss')) \
                                           .withColumnRenamed('ShiftId', 'Shift_Id') \
                                           .withColumn('Status_Start_Timestamp', from_unixtime(eqmt_status_df_filter['Timestamp'], 'yyyy-MM-dd HH:mm:ss')) \
    ...
    logger.error(TAG_MR,"TransformationException","Failed to perform join operation for table Equipment status : {}".format(str(e)))
    raise DataTransformationException(TAG_MR,"TransformationException",\
                                      "Failed to perform join operation for table Equipment status")

  tbl_Equipment_Status_df = joined_equip_status_df.select("Status_Start_Time","Shift_Id","Status_Start_Timestamp", "Duration_Seconds","Equipment","Status_Start_Time_Seconds","Delay_Reason", "Delay_Category_ID","Flag_Codes","Delay_Type_Code","Crew_Name", "Delay_Type","Comment","Operator_Id","Is_Aux","Site_Name","Src_Application","Status_End_Timestamp","Shift_Code","Shift_Date","Source_Id")
  
  return tbl_Equipment_Status_df.distinct()

# COMMAND ----------

# DBTITLE 1,Operating Entity (Site) - (Manually - Single Record MinasRio)
...
def mr_transform_tbl_delay_reasons(base_input_path, site_executor, input_config):
  site_name=input_config.site
  try:
    enum_path = get_final_input_path(base_input_path, MR_ENUM)
    enum_df = site_executor.read_data(CURATED, ADLS2, PARQUET, enum_path).filter("EnumTypeId = 19").select("Id", "Description")
    
  except Exception as e:
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {} : {}".format(enum_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format(enum_path))
  ...
  try:
    ...
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {} : {}".format(ext_reasons_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format(ext_reasons_path))
  ...
  try:
    ...
    logger.error(TAG_MR,"TransformationException","Failed to perform join operation for table Delay Reasons: {}".format(str(e)))
    raise DataTransformationException(TAG_MR,"TransformationException",\
...
# Getting Delay Reasons Dependent Entity Tables
def mr_transform_tbl_shovel_buckets(base_input_path, site_executor, input_config):
  site_name=input_config.site
  try:
    ...
    shift_load_bucket_pivot_df = site_executor.read_data(CURATED, ADLS2, PARQUET, shift_load_bucket_pivot_path) \
                                  .filter(col("ShiftId") > 171231003)
  except Exception as e:
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {} :{}".format(shift_load_bucket_pivot_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format(shift_load_bucket_pivot_path))
...
# Transforming the necessay columns and renaming the others as per the EDM. TO ASK Hesten for ShiftStartTimestamp
  try:
    ...
    logger.error(TAG_MR,"TransformationException","Failed to perform column rename operation for table Shovel Buckets: {}".format(str(e)))
    raise DataTransformationException(TAG_MR,"TransformationException",\
...
def mr_transform_tbl_material(base_input_path, site_executor, input_config):
  site_name = input_config.site
  try:
    enum_material_path = get_final_input_path(base_input_path, MR_ENUM_MATERIAL)
    enum_material_df = site_executor.read_data(CURATED, ADLS2, PARQUET, enum_material_path)
  except Exception as e:
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {} : {}".format(enum_material_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format(enum_material_path))
...
# Transforming the Material entity, no such transformations just need to rename and copy as-is
  ...
  try:
    ...
    logger.error(TAG_MR,"TransformationException","Failed to perform column rename operation for table Material: {}".format(str(e)))
    raise DataTransformationException(TAG_MR,"TransformationException",\
...
# Getting Material Entity Table
def mr_transform_tbl_truck_loads(base_input_path, site_executor, input_config):
  site_name = input_config.site
  try:
    ...
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {}: {}".format(shift_info_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format(shift_info_path))
    
  try:
    shift_loads_path = get_final_input_path(base_input_path, MR_STD_SHIFT_LOADS)
    shift_loads_df = site_executor.read_data(CURATED, ADLS2, PARQUET, shift_loads_path).filter(col("ShiftId") > 171231003)
  except Exception as e:
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {} : {}".format(shift_loads_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format(shift_loads_path))
  ...
  try:
    ...
    logger.error(TAG_MR,"ReadException",message)
    raise DataTransformationException(TAG_MR,"ReadException",message)
  ...
  try:
    ...
    logger.error(TAG_MR,"ReadException","Failed to renamed the column : {}".format(str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to renamed the column: renamed_df")
  ...
  try:
    ...
    logger.error(TAG_MR,"ReadException","Failed to select the column from {} : {}".format("tbl_truck_loads_df", str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to select the column from {}".format("tbl_truck_loads_df"))
...
def mr_transform_tbl_truck_dumps(base_input_path, site_executor, input_config):
  site_name = input_config.site
  try:
    shift_equipment_df = site_executor.read_data(None, SQL, None, STG_MR_EQUIPEMENT) \
                                      .select("Plant_No", "MDM_Global_Plant_No").distinct()
  except Exception as e:
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {}: {}".format(STG_MR_EQUIPEMENT, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from table {}".format(STG_MR_EQUIPEMENT))
  try:
    shift_dumps_path = get_final_input_path(base_input_path, MR_STD_SHIFT_DUMPS)
    shift_dumps_df = site_executor.read_data(CURATED, ADLS2, PARQUET, shift_dumps_path).filter(col("ShiftId") > 171231003)
  except Exception as e:
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {}: {}".format(shift_dumps_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format(shift_dumps_path))
  ...
  try:
    ...
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {}: {}".format(enum_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from table {}".format(enum_path))
    
  try:
    shift_info_path = get_final_input_path(base_input_path, MR_STD_SHIFT_INFO)
    shift_info_df = site_executor.read_data(CURATED, ADLS2, PARQUET, shift_info_path) \
                                     .select("ShiftId", "FullShiftSuffix")
  except Exception as e:
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {}: {}".format(shift_info_path, str(e)))
  ...
  try:
    shift_toper_path = get_final_input_path(base_input_path, MR_STD_SHIFT_TOPER)
    shift_toper_df = site_executor.read_data(CURATED, ADLS2, PARQUET, shift_toper_path) \
                                      .select("FieldId", "Id")
  except Exception as e:
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {}: {}".format(shift_toper_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format(shift_toper_path))

  try:
    shift_locations_path = get_final_input_path(base_input_path, MR_STD_SHIFT_LOCATIONS)
    shift_locations_df = site_executor.read_data(CURATED, ADLS2, PARQUET, shift_locations_path) \
                                          .select("Id", "Region")
  except Exception as e:
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {}: {}".format(shift_locations_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format(shift_locations_path))
  ...
  try:
    ...
    try:
      tbl_truck_dumps_df = renamed_df.select("Site_Name","Src_Application", "Shift_Id","Shift_Name","Dump_Id", \
                                             ...
                                             "Is_Waste","Is_Trammed","Is_Extra_Load","Load_Type_Id","Load_Type_Description")

    except Exception as e:
      logger.error(TAG_MR,"ReadException","Failed to read the data from path {}: {}".format("tbl_truck_dump_df", str(e)))
      raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from path {}".format("tbl_truck_loads_df"))
      
  except Exception as e:
    message = "Failed to perform either column renamad or new column creation : {}".format(str(e))
    logger.error(TAG_MR,"RenamedException",message)
    raise DataTransformationException(TAG_MR,"RenamedException",message)
...
# DBTITLE 1,Enum
def mr_transform_tbl_enum(base_input_path, site_executor, input_config):
  try:
    enum_path = get_final_input_path(base_input_path, MR_ENUM)
...
def mr_transform_tbl_shift_info(base_input_path, site_executor, input_config):
  logger.info(TAG_MR,"Performaing Transformation for shiftinfo")
  
  try:
    shift_info_path = get_final_input_path(base_input_path, MR_SHIFT_INFO)
    shift_info_df = site_executor.read_data(CURATED, ADLS2, PARQUET, shift_info_path)
                                      
  except Exception as e:
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {}: {}".format(shift_info_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from table {}".format(shift_info_path))
...
# DBTITLE 1,Person
def mr_transform_tbl_person(base_input_path, site_executor, input_config):
  logger.info(TAG_MR,"Performaing Transformation for person")
  
  try:
    person_path = get_final_input_path(base_input_path, MR_PERSON)
    ...
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {}: {}".format(person_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from table {}".format(person_path))
...
# DBTITLE 1,Enum CREW
def mr_transform_tbl_enum_crew(base_input_path, site_executor, input_config):
  logger.info(TAG_MR,"Performaing Transformation for enum crew")
  
  try:
    enum_crew_path = get_final_input_path(base_input_path, MR_ENUM_CREW)
    ...
    logger.error(TAG_MR,"ReadException","Failed to read the data from path {}: {}".format(enum_crew_path, str(e)))
    raise DataTransformationException(TAG_MR,"ReadException","Failed to read the data from table {}".format(enum_crew_path))
    
  return tbl_enum_crew_df.distinct()

# COMMAND ----------

class MinasRioTransformationsFms:
  def execute(self, data_dict, site_executor, input_config):
    logger.info(TAG_MR,"Excuting for site {}".format(input_config.site))
    error_count = 0

    try:
      base_input_path = site_executor.get_adls2_path(CURATED, input_config)
      logger.info(TAG_LBT,"Base_input_path : "+base_input_path)
    except DataTransformationException as e:
      logger.error(TAG_LBT, e.exeptionType, e.message)
      raise SystemExit(e.message)

    try:
      data_dict[STG_MR_EQUIPEMENT] = None
      tbl_Equipment_df = mr_transform_tbl_Equipment(base_input_path, site_executor,input_config)
      data_dict[STG_MR_EQUIPEMENT] = tbl_Equipment_df
    except DataTransformationException as e:
      logger.error(TAG_MR,"TransformationException","Failed to transform the data for table: {}".format(STG_MR_EQUIPEMENT))
      error_count += 1
      
    try:
      data_dict[STG_MR_EQUIPEMENT_STATUS] = None
      tbl_Equipment_Status_df = mr_transform_tbl_equipment_status(base_input_path, site_executor, input_config)
      data_dict[STG_MR_EQUIPEMENT_STATUS] = tbl_Equipment_Status_df
    except DataTransformationException as e:
      logger.error(TAG_MR,"TransformationException","Failed to transform the data for \
      table: {}".format(STG_MR_EQUIPEMENT_STATUS))
      error_count += 1
      
    try:
      data_dict[STG_MR_DELAY_REASON] = None
      tbl_Delay_Reasons_df = mr_transform_tbl_delay_reasons(base_input_path, site_executor, input_config)
      data_dict[STG_MR_DELAY_REASON] = tbl_Delay_Reasons_df
    except DataTransformationException as e:
      logger.error(TAG_MR,"TransformationException","Failed to transform the data for table: {}".format(STG_MR_DELAY_REASON))
      error_count += 1
      
    try:
      data_dict[STG_MR_SHOVEL_BUCKETS] = None
      tbl_Shovel_Buckets_df = mr_transform_tbl_shovel_buckets(base_input_path, site_executor, input_config)
      data_dict[STG_MR_SHOVEL_BUCKETS] = tbl_Shovel_Buckets_df
    except DataTransformationException as e:
      logger.error(TAG_MR,"TransformationException","Failed to transform the data for table: {}".format(STG_MR_SHOVEL_BUCKETS))
      error_count += 1
      
    try:
      data_dict[STG_MR_MATERIAL] = None
      tbl_Material_df = mr_transform_tbl_material(base_input_path, site_executor, input_config)
      data_dict[STG_MR_MATERIAL] = tbl_Material_df
    except DataTransformationException as e:
      logger.error(TAG_MR,"TransformationException","Failed to transform the data for table: {}".format(STG_MR_MATERIAL))
      error_count += 1
      
    try:
      data_dict[STG_MR_TRUCK_LOADS] = None
      tbl_Truck_Loads_df = mr_transform_tbl_truck_loads(base_input_path, site_executor, input_config)
      data_dict[STG_MR_TRUCK_LOADS] = tbl_Truck_Loads_df      
    except DataTransformationException as e:
      logger.error(TAG_MR,"TransformationException","Failed to transform the data for table: {}".format(STG_MR_TRUCK_LOADS) + "\n" + e.message)
      error_count += 1
      
    try:
      data_dict[STG_MR_TRUCK_DUMPS] = None
      tbl_Truck_Dumps_df = mr_transform_tbl_truck_dumps(base_input_path, site_executor, input_config)
      data_dict[STG_MR_TRUCK_DUMPS] = tbl_Truck_Dumps_df      
    except DataTransformationException as e:
      logger.error(TAG_MR,"TransformationException","Failed to transform the data for table: {}".format(STG_MR_TRUCK_DUMPS) + "\n" + e.message)
      error_count += 1
      
    try:
      data_dict[STG_MR_ENUM] = None
      tbl_Enum_df = mr_transform_tbl_enum(base_input_path, site_executor, input_config)
      data_dict[STG_MR_ENUM] = tbl_Enum_df      
    except DataTransformationException as e:
      logger.error(TAG_MR,"TransformationException","Failed to transform the data for table: {}".format(STG_MR_ENUM) + "\n" + e.message)
      error_count += 1
      
    try:
      data_dict[STG_MR_SHIFT_INFO] = None
      tbl_shift_info_df = mr_transform_tbl_shift_info(base_input_path, site_executor, input_config)
      data_dict[STG_MR_SHIFT_INFO] = tbl_shift_info_df      
    except DataTransformationException as e:
      logger.error(TAG_MR,"TransformationException","Failed to transform the data for table: {}".format(STG_MR_SHIFT_INFO) + "\n" + e.message)
      error_count += 1
    
    try:
      data_dict[STG_MR_PERSON] = None
      tbl_person_df = lb_transform_tbl_person(base_input_path, site_executor, input_config)
      data_dict[STG_MR_PERSON] = tbl_person_df      
    except DataTransformationException as e:
      logger.error(TAG_MR,"TransformationException","Failed to transform the data for table: {}".format(STG_MR_PERSON) + "\n" + e.message)
      error_count += 1
      
    try:
      data_dict[STG_MR_ENUM_CREW] = None
      tbl_enum_crew_df = mr_transform_tbl_enum_crew(base_input_path, site_executor, input_config)
      data_dict[STG_MR_ENUM_CREW] = tbl_enum_crew_df      
    except DataTransformationException as e:
      logger.error(TAG_MR,"TransformationException","Failed to transform the data for table: {}".format(STG_MR_ENUM_CREW) + "\n" + e.message)
      error_count += 1
    
    if error_count == 11:
      raise SystemExit("Tansformation failed for all the table for site Sishen entity Shovels")
