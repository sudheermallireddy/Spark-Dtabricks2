...
TAG_KMT = "KolomelaTransformationsFms"
...
def km_transform_tbl_Equipment(base_input_path, site_executor, input_config):
  ...
  logger.info(TAG_KMT,"reference_input_path : "+reference_input_path)
  
  try:
    aux_path = get_final_input_path(base_input_path, KM_STD_SHIFT_AUX)
    ...
    std_shift_aux_df = std_shift_aux_raw_df.withColumn("UnitType", std_shift_aux_raw_df.Unit) \
                                           .select("FieldId", "UnitId", "Unit", "UnitType", "EqmttypeId", "Eqmttype","Src_Application","Site_Name")
  except Exception as e:
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {} : {}".format(aux_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format(aux_path))

  try:
    eqmt_path = get_final_input_path(base_input_path, KM_STD_SHIFT_EQMT)
    std_shift_eqmt_df = site_executor.read_data(CURATED, ADLS2, PARQUET, eqmt_path)\
                                     .select("FieldId", "UnitId", "Unit", "UnitType", "EqmttypeId", "Eqmttype","Src_Application","Site_Name")
  except Exception as e:
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {} : {}".format(eqmt_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format(eqmt_path))
  ...
  try:
    ...
    logger.error(TAG_KMT,"TransformationException","Failed to perform union operation for table Equipment : {}".format(str(e)))
    raise DataTransformationException(TAG_KMT,"TransformationException",\
  ...
  try:
    ...
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {} : {}".format(catalog_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format(catalog_path))
  ...
  try:
    ...
    logger.error(TAG_KMT,"TransformationException","Failed to perform join operation for table Equipment : {}".format(str(e)))
    raise DataTransformationException(TAG_KMT,"TransformationException",\
...
def km_transform_tbl_equipment_status(base_input_path, site_executor, input_config):
  site_name = input_config.site
  # Getting Equipment Status tables
  eqmt_status_path = get_final_input_path(base_input_path, KM_STD_SHIFT_STATE)
  eqmt_status_df = site_executor.read_data(CURATED, ADLS2, PARQUET, eqmt_status_path)
  eqmt_status_df.select("FieldId","ShiftId","Timestamp","Duration","Eqmt",\
                        "AuxEqmt","FieldTime","FieldReason","TimecatId","FlagCodes",\
                        "StatusIdx","Status","FieldComment","OperId","IsAuxil")
  shift_info_df = km_transform_tbl_shift_info(base_input_path, site_executor, input_config).where(col("ShiftId").isNotNull()) .select('ShiftId','Crew').withColumnRenamed('ShiftId', 'ShiftId_df')
  ...
  try:
    ...
    logger.error(TAG_KMT,"TransformationException","Failed to perform join operation for table Equipment status : {}".format(str(e)))
    raise DataTransformationException(TAG_KMT,"TransformationException",\
...
# DBTITLE 1,Operating Entity (Site) - (Manually - Single Record Kolomela)
...
def km_transform_tbl_delay_reasons(base_input_path, site_executor, input_config):
  site_name = input_config.site
  try:
    enum_path = get_final_input_path(base_input_path, KM_ENUM)
    enum_df = site_executor.read_data(CURATED, ADLS2, PARQUET, enum_path).filter("EnumTypeId = 19").select("Id", "Description")
    
  except Exception as e:
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {} : {}".format(enum_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format(enum_path))
  ...
  try:
    ...
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {} : {}".format(ext_reasons_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format(ext_reasons_path))
  ...
  try:
    ...
    logger.error(TAG_KMT,"TransformationException","Failed to perform join operation for table Delay Reasons: {}".format(str(e)))
    raise DataTransformationException(TAG_KMT,"TransformationException",\
...
# Getting Delay Reasons Dependent Entity Tables
def km_transform_tbl_shovel_buckets(base_input_path, site_executor, input_config):
  site_name = input_config.site
  try:
    ...
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {} :{}".format(shift_load_bucket_pivot_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format(shift_load_bucket_pivot_path))
...
# Transforming the necessay columns and renaming the others as per the EDM. TO ASK Hesten for ShiftStartTimestamp
  try:
    ...
    logger.error(TAG_KMT,"TransformationException","Failed to perform column rename operation for table Shovel Buckets: {}".format(str(e)))
    raise DataTransformationException(TAG_KMT,"TransformationException",\
...
def km_transform_tbl_material(base_input_path, site_executor, input_config):
  site_name = input_config.site
  try:
    enum_material_path = get_final_input_path(base_input_path, KM_ENUM_MATERIAL)
    enum_material_df = site_executor.read_data(CURATED, ADLS2, PARQUET, enum_material_path)
  except Exception as e:
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {} : {}".format(enum_material_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format(enum_material_path))
...
# Transforming the Material entity, no such transformations just need to rename and copy as-is
  ...
  try:
    ...
    logger.error(TAG_KMT,"TransformationException","Failed to perform column rename operation for table Material: {}".format(str(e)))
    raise DataTransformationException(TAG_KMT,"TransformationException",\
...
# Getting Material Entity Table
def km_transform_tbl_truck_loads(base_input_path, site_executor, input_config):
  site_name = input_config.site
  try:
    ...
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {}: {}".format(shift_info_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format(shift_info_path))
  ...
  try:
    ...
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {} : {}".format(shift_loads_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format(shift_loads_path))
  ...
  try:
    ...
    logger.error(TAG_KMT,"ReadException",message)
    raise DataTransformationException(TAG_KMT,"ReadException",message)
  ...
  try:
    ...
    logger.error(TAG_KMT,"ReadException","Failed to renamed the column : {}".format(str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to renamed the column: renamed_df")
  ...
  try:
    ...
    logger.error(TAG_KMT,"ReadException","Failed to select the column from {} : {}".format("tbl_truck_loads_df", str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to select the column from {}".format("tbl_truck_loads_df"))
...
def km_transform_tbl_truck_dumps(base_input_path, site_executor, input_config):
  site_name = input_config.site

  try:
    shift_equipment_df = site_executor.read_data(None, SQL, None, STG_KM_EQUIPEMENT) \
                                      .select("Plant_No", "MDM_Global_Plant_No")
  except Exception as e:
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {}: {}".format(STG_KM_EQUIPEMENT, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from table {}".format(STG_KM_EQUIPEMENT))
  ...
  try:
    ...
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {}: {}".format(enum_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from table {}".format(enum_path))
    
  try:
    shift_dumps_path = get_final_input_path(base_input_path, KM_STD_SHIFT_DUMPS)
    shift_dumps_df = site_executor.read_data(CURATED, ADLS2, PARQUET, shift_dumps_path)
  except Exception as e:
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {}: {}".format(shift_dumps_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format(shift_dumps_path))                              
  try:
    shift_info_path = get_final_input_path(base_input_path, KM_STD_SHIFT_INFO)
    shift_info_df = site_executor.read_data(CURATED, ADLS2, PARQUET, shift_info_path) \
                                     .select("ShiftId", "FullShiftSuffix")
  except Exception as e:
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {}: {}".format(shift_info_path, str(e)))
  ...
  try:
    shift_toper_path = get_final_input_path(base_input_path, KM_STD_SHIFT_TOPER)
    shift_toper_df = site_executor.read_data(CURATED, ADLS2, PARQUET, shift_toper_path) \
                                      .select("FieldId", "Id")
  except Exception as e:
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {}: {}".format(shift_toper_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format(shift_toper_path))

  try:
    shift_locations_path = get_final_input_path(base_input_path, KM_STD_SHIFT_LOCATIONS)
    shift_locations_df = site_executor.read_data(CURATED, ADLS2, PARQUET, shift_locations_path) \
                                          .select("Id", "Region")
  except Exception as e:
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {}: {}".format(shift_locations_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format(shift_locations_path))
  ...
  try:
    ...
    try:
      ...
      logger.error(TAG_KMT,"ReadException","Failed to read the data from path {}: {}".format("tbl_truck_dump_df", str(e)))
      raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from path {}".format("tbl_truck_loads_df"))
      
  except Exception as e:
    message = "Failed to perform either column renamad or new column creation : {}".format(str(e))
    logger.error(TAG_KMT,"RenamedException",message)
    raise DataTransformationException(TAG_KMT,"RenamedException",message)
...
# DBTITLE 1,Enum
def km_transform_tbl_enum(base_input_path, site_executor, input_config):
  site_name = input_config.site
  try:
    enum_path = get_final_input_path(base_input_path, KM_ENUM)
...
def km_transform_tbl_shift_info(base_input_path, site_executor, input_config):
  logger.info(TAG_KMT,"Performaing Transformation for shiftinfo")
  
  try:
    shift_info_path = get_final_input_path(base_input_path, KM_SHIFT_INFO)
    shift_info_df = site_executor.read_data(CURATED, ADLS2, PARQUET, shift_info_path)
                                      
  except Exception as e:
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {}: {}".format(shift_info_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from table {}".format(shift_info_path))
...
# DBTITLE 1,Person
def km_transform_tbl_person(base_input_path, site_executor, input_config):
  logger.info(TAG_KMT,"Performaing Transformation for person")
  
  try:
    person_path = get_final_input_path(base_input_path, KM_PERSON)
    ...
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {}: {}".format(person_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from table {}".format(person_path))
...
# DBTITLE 1,Enum CREW
def km_transform_tbl_enum_crew(base_input_path, site_executor, input_config):
  logger.info(TAG_KMT,"Performaing transformation for enum crew")
  
  try:
    enum_crew_path = get_final_input_path(base_input_path, KM_ENUM_CREW)
    ...
    logger.error(TAG_KMT,"ReadException","Failed to read the data from path {}: {}".format(enum_crew_path, str(e)))
    raise DataTransformationException(TAG_KMT,"ReadException","Failed to read the data from table {}".format(enum_crew_path))
    
  return tbl_enum_crew_df.distinct()

# COMMAND ----------

class KolomelaTransformationsFms:
  def execute(self, data_dict, site_executor, input_config):
    logger.info(TAG_KMT,"Excuting for site {}".format(input_config.site))
    error_count = 0
    ...
    try:
      data_dict[STG_KM_EQUIPEMENT] = None
      tbl_Equipment_df = km_transform_tbl_Equipment(base_input_path, site_executor,input_config)
      data_dict[STG_KM_EQUIPEMENT] = tbl_Equipment_df
    except DataTransformationException as e:
      logger.error(TAG_KMT,"TransformationException","Failed to transform the data for table: {}".format(STG_KM_EQUIPEMENT))
      error_count += 1
      
    try:
      data_dict[STG_KM_EQUIPEMENT_STATUS] = None
      tbl_Equipment_Status_df = km_transform_tbl_equipment_status(base_input_path, site_executor, input_config)
      data_dict[STG_KM_EQUIPEMENT_STATUS] = tbl_Equipment_Status_df
    except DataTransformationException as e:
      logger.error(TAG_KMT,"TransformationException","Failed to transform the data for \
      table: {}".format(STG_KM_EQUIPEMENT_STATUS))
      error_count += 1
      
    try:
      data_dict[STG_KM_DELAY_REASON] = None
      tbl_Delay_Reasons_df = km_transform_tbl_delay_reasons(base_input_path, site_executor, input_config)
      data_dict[STG_KM_DELAY_REASON] = tbl_Delay_Reasons_df
    except DataTransformationException as e:
      logger.error(TAG_KMT,"TransformationException","Failed to transform the data for table: {}".format(STG_KM_DELAY_REASON))
      error_count += 1
      
    try:
      data_dict[STG_KM_SHOVEL_BUCKETS] = None
      tbl_Shovel_Buckets_df = km_transform_tbl_shovel_buckets(base_input_path, site_executor, input_config)
      data_dict[STG_KM_SHOVEL_BUCKETS] = tbl_Shovel_Buckets_df
    except DataTransformationException as e:
      logger.error(TAG_KMT,"TransformationException","Failed to transform the data for table: {}".format(STG_KM_SHOVEL_BUCKETS))
      error_count += 1
      
    try:
      data_dict[STG_KM_MATERIAL] = None
      tbl_Material_df = km_transform_tbl_material(base_input_path, site_executor, input_config)
      data_dict[STG_KM_MATERIAL] = tbl_Material_df
    except DataTransformationException as e:
      logger.error(TAG_KMT,"TransformationException","Failed to transform the data for table: {}".format(STG_KM_MATERIAL))
      error_count += 1
      
    try:
      data_dict[STG_KM_TRUCK_LOADS] = None
      tbl_Truck_Loads_df = km_transform_tbl_truck_loads(base_input_path, site_executor, input_config)
      data_dict[STG_KM_TRUCK_LOADS] = tbl_Truck_Loads_df      
    except DataTransformationException as e:
      logger.error(TAG_KMT,"TransformationException","Failed to transform the data for table: {}".format(STG_KM_TRUCK_LOADS) + "\n" + e.message)
      error_count += 1
      
    try:
      data_dict[STG_KM_TRUCK_DUMPS] = None
      tbl_Truck_Dumps_df = km_transform_tbl_truck_dumps(base_input_path, site_executor, input_config)
      data_dict[STG_KM_TRUCK_DUMPS] = tbl_Truck_Dumps_df      
    except DataTransformationException as e:
      logger.error(TAG_KMT,"TransformationException","Failed to transform the data for table: {}".format(STG_KM_TRUCK_DUMPS) + "\n" + e.message)
      error_count += 1
      
    try:
      data_dict[STG_KM_ENUM] = None
      tbl_Enum = km_transform_tbl_enum(base_input_path, site_executor, input_config)
      data_dict[STG_KM_ENUM] = tbl_Enum      
    except DataTransformationException as e:
      logger.error(TAG_KMT,"TransformationException","Failed to transform the data for table: {}".format(STG_KM_TRUCK_DUMPS) + "\n" + e.message)
      error_count += 1
    
    try:
      data_dict[STG_KM_SHIFT_INFO] = None
      tbl_shift_info_df = km_transform_tbl_shift_info(base_input_path, site_executor, input_config)
      data_dict[STG_KM_SHIFT_INFO] = tbl_shift_info_df      
    except DataTransformationException as e:
      logger.error(TAG_KMT,"TransformationException","Failed to transform the data for table: {}".format(STG_KM_SHIFT_INFO) + "\n" + e.message)
      error_count += 1
      
    try:
      data_dict[STG_KM_PERSON] = None
      tbl_person_df = km_transform_tbl_person(base_input_path, site_executor, input_config)
      data_dict[STG_KM_PERSON] = tbl_person_df      
    except DataTransformationException as e:
      logger.error(TAG_T,"TransformationException","Failed to transform the data for table: {}".format(STG_KM_PERSON) + "\n" + e.message)
      error_count += 1
      
    try:
      data_dict[STG_KM_ENUM_CREW] = None
      tbl_enum_crew_df = km_transform_tbl_enum_crew(base_input_path, site_executor, input_config)
      data_dict[STG_KM_ENUM_CREW] = tbl_enum_crew_df      
    except DataTransformationException as e:
      logger.error(TAG_KMT,"TransformationException","Failed to transform the data for table: {}".format(STG_KM_ENUM_CREW) + "\n" + e.message)
      error_count += 1
   
    if error_count == 11:
      raise SystemExit("Tansformation failed for all the table for site Kolomela process FMS")
